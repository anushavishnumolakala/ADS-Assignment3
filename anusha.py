...
def filter_Mortality_rate_data(filename, countries, indicators, start_year, end_year):
    """
    ...
    """
    # read the CSV file and skip the first 4 rows
    Mortality_rate_data = pd.read_csv(filename, skiprows=4)

    # drop unnecessary columns
    cols_to_drop = ['Country Code', 'Indicator Code', 'Unnamed: 66']
    Mortality_rate_data = Mortality_rate_data.drop(cols_to_drop, axis=1)

    # rename remaining columns
    Mortality_rate_data = Mortality_rate_data.rename(
        columns={'Country Name': 'Country'})

    # filter data by selected countries and indicators
    Mortality_rate_data = Mortality_rate_data[Mortality_rate_data['Country'].isin(countries) &
                                                  Mortality_rate_data['Indicator Name'].isin(indicators)]

    # melt the dataframe to convert years to a single column
    Mortality_rate_data = Mortality_rate_data.melt(id_vars=['Country', 'Indicator Name'],
                                                       var_name='Year', value_name='Value')

    # convert year column to integer and value column to float
    Mortality_rate_data['Year'] = pd.to_numeric(
        Mortality_rate_data['Year'], errors='coerce')
    Mortality_rate_data['Value'] = pd.to_numeric(
        Mortality_rate_data['Value'], errors='coerce')

    # pivot the dataframe to create a single dataframe with years as columns and countries and indicators as rows
    Mortality_rate_data = Mortality_rate_data.pivot_table(index=['Country', 'Indicator Name'],
                                                              columns='Year', values='Value')

    # select specific years
    Mortality_rate_data = Mortality_rate_data.loc[:, start_year:end_year]

    return Mortality_rate_data
...
def predict_future(Mortality_rate_data, countries, indicators, start_year, end_year):
    # select data for the given countries, indicators, and years
    data = filter_Mortality_rate_data(Mortality_rate_data, countries,
...
if __name__ == '__main__':
    ...
    indicators = [
        'Mortality rate, under-5 (per 1,000 live births)', 'Population growth (annual %)']
    countries = ['India', 'United States', 'Bangladesh', 'China']
